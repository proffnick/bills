...
const PayChurch = ({ image, data, process  }) => {
  ...
  const act = () => {
    try {
      ...
      process({member, amount, selected});
  ...
  return(
    ...
    <SelectInput
      id={`data_bubdle`}
      data={data} 
      label={`Select ${data?.[0]?.name} service`}
    ...
    <TextInput 
      name={`pay_church`}
      label={`Enter Member ${data?.[0]?.label_name}`}
      type={`number`}
      action={setmember} 
    />
